...
def test_write_json_serializes_concurrent_writes(monkeypatch):
    ...
    # Fork: a deferred session-build thread from an earlier test can emit a
    # stray session.info event through the patched stdout (~2s after
    # session.create). Count only THIS test's writes — each must still be an
    # intact, unfragmented JSON line, which is what the serialization lock
    # guarantees.
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # Fork: "session" (session_search/session_usage) is a fork platform
    # toolset recovered from the CLI universe alongside kanban.
    assert set(result) - {"kanban", "memory", "project", "session"} <= _RECENTLY_SHIPPED_TOOLSETS
...
# ---------------------------------------------------------------------------
# Fork tests (hermes-custom): Skills category in the command catalog, and the
# persona field on session.info.
# ---------------------------------------------------------------------------
...
def test_session_info_includes_persona(monkeypatch):
    monkeypatch.setattr(
