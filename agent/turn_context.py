...
def compose_user_api_content(
    ...
    context_engine_prefetch: str = "",
) -> Optional[str]:
    """Compose the API-bound content of the current turn's user message.

    Sources: memory-manager prefetch + ``pre_llm_call`` plugin context with
    target="user_message" (the default) + the context engine's per-turn
    ``prefetch()`` result (fork extension — e.g. the LifeOS engine's vault
    operating context). All are appended to the *API copy* of the user
    ...
    if context_engine_prefetch:
        injections.append(context_engine_prefetch)
    if not injections:
        return None
    return content + "\n\n" + "\n\n".join(injections)
...
class TurnContext:
    ...
    ext_prefetch_cache: str = ""
    # Context-engine per-turn prefetch (fork: e.g. LifeOS vault context),
    # composed into the user message's API copy alongside the sources above.
    context_engine_prefetch: str = ""
...
def build_turn_context(
    ...
    *,
    ...
    # Context engine prefetch (fork): lets a pluggable engine inject
    # domain-specific operating context into the current user turn without
    # mutating the cached system prompt or waiting for compression to fire.
    # Composed into the API copy via compose_user_api_content below, so the
    # byte-stable api_content sidecar contract holds for this source too.
    context_engine_prefetch = ""
    ...
    return TurnContext(
        ...
        context_engine_prefetch=context_engine_prefetch,
