...
class HonchoSessionManager:
    ...
    def search_context(
        ...
        # Federation: ALSO consult the read-only LIFE host (work reads life,
        # read-down). Life must stay on the context/representation path — the
        # :8010 proxy denies /messages, so it can never join the message
        # search above. Runs even when the local search came up empty.
        try:
            ...
            if life.get("card"):
                life_parts.append("\n".join(f"- {f}" for f in life["card"]))
            if life_parts:
                life_block = "[life memory]\n" + "\n\n".join(life_parts)
                out = f"{out}\n\n{life_block}" if out else life_block
        except Exception as e:
            logger.debug("Life host search_context fan-out failed: %s", e)
