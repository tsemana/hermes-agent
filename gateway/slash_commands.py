...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            # Cost estimation. Prefer the accumulated session cost produced by
            # the conversation loop/tool rollups; fall back to a local estimate
            # only when no real session total is available.
            accumulated_cost = getattr(agent, "session_estimated_cost_usd", None)
            accumulated_status = getattr(agent, "session_cost_status", None) or "estimated"
            if isinstance(accumulated_cost, (int, float)) and accumulated_cost > 0:
                prefix = "~" if accumulated_status == "estimated" else ""
                lines.append(t("gateway.usage.label_cost", prefix=prefix, amount=f"{float(accumulated_cost):.4f}"))
            else:
                try:
                    ...
                    cost_result = estimate_usage_cost(
                        ...
                        CanonicalUsage(
                            ...
                            request_count=getattr(agent, "session_api_calls", 1) or 1,
                        ),
